...
public:
    ...
    // z거리를 측정해서 한 평면에 있으면 제거함
    void check_z(pcl::PointCloud<pcl::PointXYZI>& store, pcl::PointCloud<pcl::PointXYZI>& input)
    {
        pcl::PointXYZI minPoint, maxPoint;
        pcl::getMinMax3D(input, minPoint, maxPoint);

        float c_distance = abs(maxPoint.z - minPoint.z); // 높이차이가 별로 없다면

        if (c_distance > 0.05)  // 높이 차이가 0.05이면 같은 평면임
        {
            store += input;
        }
    }
